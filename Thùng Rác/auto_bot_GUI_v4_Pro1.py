...
def main_gui():
    ...
    def show_help():
        messagebox.showinfo(
            "Hướng dẫn",
            "V4_pro:\n- Start/Stop/Pause/Resume\n- Dashboard mini hiển thị: xanh=thắng, đỏ=thua, xanh dương=đặt\n- DRY_RUN: bật để thử mà không click chuột\n- Có thể lưu/nạp profile trong thư mục profiles/\n- Có beep âm thanh (Windows) khi thắng/thua nếu bật\n- --selftest để kiểm thử nhanh không cần GUI"
        )
    ...
    ttk.Label(frame, text="🤖 Trạng thái bot:", font=('Arial', 11, 'bold')).pack(pady=(0,4))
    status_label = ttk.Label(frame, textvariable=status_var, style="Status.TLabel")
    status_label.pack()

    ttk.Label(frame, textvariable=bet_var, font=('Arial', 10)).pack(pady=4)

    # Dashboard sparkline
    ttk.Label(frame, text="📊 Dashboard (60 gần nhất)", font=('Arial', 10, 'bold')).pack(pady=(10,4))
    spark_canvas = tk.Canvas(frame, width=520, height=80, bg="#1e1e1e", highlightthickness=1, highlightbackground="#444")
    spark_canvas.pack(pady=(0,8))

    # Log box
    ttk.Label(frame, text="🧭 Nhật ký", font=('Arial', 10, 'bold')).pack(pady=(6,4))
    log_box = scrolledtext.ScrolledText(frame, width=68, height=12, font=('Consolas', 9))
    log_box.pack(pady=4)

    # --- Các ô tọa độ ---
    ttk.Label(frame, text="⚙️ Cập nhật tọa độ:", font=('Arial', 10, 'bold')).pack(pady=(10,4))
    ...
    def make_entry(label, key, pos):
        ttk.Label(coord_frame, text=label).pack()
        e = ttk.Entry(coord_frame, width=32)
    ...
    ttk.Button(coord_frame, text="📍 Xác định & Gán tọa độ (2s)", command=lambda: pick_position(focused_entry, widgets, status_var, root)).pack(pady=6)

    # --- Delay / Tùy chọn ---
    ttk.Label(frame, text="🕒 Cấu hình thời gian chờ:", font=('Arial', 10, 'bold')).pack(pady=(10,4))
    ...
    e_delay_bet = ttk.Entry(delay_frame, textvariable=var_delay_bet, width=12)
    e_delay_bet.pack(pady=2)

    ttk.Label(delay_frame, text="📩 Sau khi thắng chờ (giây):").pack()
    e_delay_win = ttk.Entry(delay_frame, textvariable=var_delay_win, width=12)
    e_delay_win.pack(pady=2)

    # --- Khác: DRY_RUN, Âm thanh, Tesseract path, Levels ---
    misc_frame = ttk.Labelframe(frame, text="Tùy chọn nâng cao", padding=8)
    ...
    c1 = ttk.Checkbutton(misc_frame, text="DRY_RUN (không click thật)", variable=var_dry)
    c1.pack(anchor='w')
    c2 = ttk.Checkbutton(misc_frame, text="Âm thanh Thắng/Thua (Windows)", variable=var_sound)
    c2.pack(anchor='w')

    ttk.Label(misc_frame, text="Đường dẫn Tesseract:").pack(anchor='w', pady=(6,2))
    entry_tess = ttk.Entry(misc_frame, width=58)
    entry_tess.insert(0, pytesseract.pytesseract.tesseract_cmd)
    entry_tess.pack(anchor='w')

    ttk.Label(misc_frame, text="Dãy hệ số (phân cách bằng dấu phẩy):").pack(anchor='w', pady=(6,2))
    entry_levels = ttk.Entry(misc_frame, width=58)
    entry_levels.insert(0, ",".join(map(str, BET_LEVELS)))
    entry_levels.pack(anchor='w')

    # lưu references cho load_config
    widgets.update({
    ...
    # --- Nút cập nhật ---
    btns_cfg = ttk.Frame(frame)
    btns_cfg.pack(pady=6)
    ttk.Button(btns_cfg, text="💾 Cập nhật & Lưu (config.json)", command=lambda: apply_updates(widgets, status_var, save=True, path=DEFAULT_CONFIG)).pack(side='left', padx=6)
    ttk.Button(btns_cfg, text="🔄 Chỉ cập nhật tạm", command=lambda: apply_updates(widgets, status_var, save=False)).pack(side='left', padx=6)

    # --- Điều khiển ---
    ctrl = ttk.Frame(frame)
    ctrl.pack(pady=10)

    ttk.Button(ctrl, text="▶ Start", command=lambda: start_bot(status_var, bet_var, log_box, spark_canvas)).pack(side='left', padx=6)
    ...
    # helper để vẽ sparkline ban đầu
