...
# ============================
#  Auto Bot Winluck v4_pro (Compact UI+)
#  - Bố cục 2 cột, gọn hơn
#  - Nút "Cập nhật & Lưu (config.json)"
#  - Toggle thông báo Thắng/Thua (âm thanh + popup)
#  - Nút "Mở Logs" và "Thoát"
#  - Pause / Resume / Stop
#  - Dashboard sparkline không dùng matplotlib
#  - DRY_RUN, chọn Tesseract, lưu/nạp profile
#  - --selftest: kiểm thử không cần GUI
# ============================
...
def play_sound(win=True):
    ...
    try:
        if os.name == 'nt':
            import winsound
            winsound.Beep(1200 if win else 500, 120 if win else 200)
    except Exception as e:
        logging.warning(f"Âm thanh lỗi: {e}")

def popup_notify(text):
    if not ENABLE_NOTIFY:
        return
    try:
        # Hiện thông báo nhỏ, không chặn luồng chính lâu
...
def get_text_from_region(region):
    try:
        ...
        arr = cv2.cvtColor(np.array(im.convert('RGB')), cv2.COLOR_BGR2GRAY)
        _, th = cv2.threshold(arr, 180, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        pil = Image.fromarray(th)
        text = pytesseract.image_to_string(pil, lang=TESSERACT_LANG, config='--psm 6')
...
def draw_sparkline(canvas):
    ...
    x = w - step*(n-1)
    pts = [(x + i*step, y_map(v)) for i, v in enumerate(history)]
...
def bot_loop(status_var, bet_var, log_box, spark_canvas):
    ...
    set_status(status_var, "🔄 Đang chạy...")

    while bot_running:
        try:
            while bot_paused and bot_running:
                set_status(status_var, "⏸ Paused — bot đang tạm dừng")
                time.sleep(0.3)
            if not bot_running:
                break
            ...
            if "THẮNG" in text:
                ...
                if waiting_for_win:
                    if bet_index < len(BET_LEVELS) - 1:
                        bet_index += 1
                    log_msg(f"✅ THẮNG (trong chế độ chờ) → tăng hệ số {BET_LEVELS[bet_index]}")
                    push_history(1, spark_canvas)
                    waiting_for_win = False
                    last_prediction = None
                    countdown(status_var, "⏳ Chờ Telegram cập nhật...", DELAY_AFTER_WIN_WAIT)
                    continue
                else:
            ...
            if waiting_for_win:
                set_status(status_var, "⏳ Đang chờ tín hiệu THẮNG để tăng hệ số...")
                time.sleep(0.25)
                continue
            ...
            if "LỚN" in text or "LON" in text:
                if last_prediction != "LON":
                    set_status(status_var, f"⬆️ Đánh LỚN ({BET_LEVELS[bet_index]})")
                ...
                if last_prediction != "NHO":
                    set_status(status_var, f"⬇️ Đánh NHỎ ({BET_LEVELS[bet_index]})")
            ...
            set_status(status_var, f"⚠️ Lỗi: {e}")
            time.sleep(1.0)

    set_status(status_var, "⏹ Bot đã dừng")

def countdown(status_var, label, secs):
    for i in range(secs, 0, -1):
        set_status(status_var, f"{label} {i}s")
        time.sleep(1)
...
def load_config_from(path, widgets, status_var):
    ...
    global DELAY_AFTER_BET, DELAY_AFTER_WIN_WAIT, DRY_RUN, ENABLE_SOUND, ENABLE_NOTIFY, BET_LEVELS
    try:
        ...
        ENABLE_NOTIFY = bool(data.get("ENABLE_NOTIFY", ENABLE_NOTIFY))
        ...
        widgets["var_notify"].set(1 if ENABLE_NOTIFY else 0)
...
def apply_updates(widgets, status_var, save=False, path=DEFAULT_CONFIG):
    ...
    try:
        ...
        ENABLE_NOTIFY = bool(widgets["var_notify"].get())
...
class _FakeCanvas:
    def __init__(self, w=200, h=80):
        ...
        self.lines = []; self.cleared = False
    def __getitem__(self, k): return self.cfg[k]
    def delete(self, *_): self.cleared = True
...
def main_gui():
    ...
    def m_save_config():
        path = filedialog.asksaveasfilename(initialdir="profiles", defaultextension=".json",
                                            filetypes=[("JSON","*.json")])
        if path: save_config_to(path, status_var)

    def m_load_config():
        path = filedialog.askopenfilename(initialdir="profiles", filetypes=[("JSON","*.json")])
        if path: load_config_from(path, widgets, status_var)
    ...
    def m_open_logs():
        try:
            ...
            status_var.set("📂 Đã mở thư mục logs")
        except Exception as e:
            status_var.set(f"⚠️ Không mở được thư mục logs: {e}")

    def show_help():
        messagebox.showinfo("Hướng dẫn",
            "V4_pro (Compact UI+):\n- Start/Stop/Pause/Resume\n- Dashboard sparkline\n"
            "- DRY_RUN, Âm thanh, Popup thông báo\n- Lưu/Nạp profile trong profiles/\n"
            "- 'Cập nhật & Lưu (config.json)' để ghi nhanh cấu hình")

    filemenu.add_command(label="Save Config (Profile)", command=m_save_config)
    filemenu.add_command(label="Load Config (Profile)", command=m_load_config)
    filemenu.add_separator()
    filemenu.add_command(label="Quit", command=root.destroy)
    viewmenu.add_command(label="Open Logs Folder", command=m_open_logs)
    helpmenu.add_command(label="About / Help", command=show_help)
    ...
    spark_canvas = tk.Canvas(left_frame, width=480, height=90, bg="#1e1e1e",
                             highlightthickness=1, highlightbackground="#444")
    spark_canvas.pack(pady=(0, 10))

    ttk.Label(left_frame, text="🧭 Nhật ký", font=('Arial', 10, 'bold')).pack(pady=(4, 4))
    log_box = scrolledtext.ScrolledText(left_frame, width=70, height=18, font=('Consolas', 9))
    ...
    widgets = {}
    focused_entry = tk.StringVar(value="")

    def make_entry(label, key, pos):
        f = ttk.Frame(coord_frame)
        f.pack(pady=2, anchor='w')
        ttk.Label(f, text=label, width=12).pack(side='left')
        e = ttk.Entry(f, width=20)
        e.insert(0, f"{pos[0]}, {pos[1]}")
        e.pack(side='left')
        e.bind("<FocusIn>", lambda _ev, k=key: focused_entry.set(k))
        widgets[key] = e

    make_entry("Nút LỚN:", "entry_lon", BTN_LON_POS)
    make_entry("Nút NHỎ:", "entry_nho", BTN_NHO_POS)
    make_entry("Ô BET:", "entry_bet", BET_BOX_POS)
    make_entry("Nút GỬI:", "entry_gui", BTN_GUI_POS)
    make_entry("Telegram:", "entry_tele", TELE_CLICK_POS)
    make_entry("Đóng cược:", "entry_close", TELE_CLOSE_POS)

    ttk.Button(right_frame, text="📍 Gán tọa độ (2s)",
               command=lambda: pick_position(focused_entry, widgets, status_var, root)).pack(pady=6)

    ttk.Label(right_frame, text="🕒 Thời gian chờ", font=('Arial', 10, 'bold')).pack(pady=(8, 4))
    ...
    # --- Tùy chọn nâng cao ---
    misc_frame = ttk.Labelframe(right_frame, text="Tùy chọn nâng cao", padding=8)
    misc_frame.pack(fill='x', pady=8)
    var_dry = tk.IntVar(value=1 if DRY_RUN else 0)
    var_sound = tk.IntVar(value=1 if ENABLE_SOUND else 0)
    var_notify = tk.IntVar(value=1 if ENABLE_NOTIFY else 0)
    ttk.Checkbutton(misc_frame, text="DRY_RUN (không click thật)", variable=var_dry).pack(anchor='w')
    ttk.Checkbutton(misc_frame, text="Âm thanh Thắng/Thua", variable=var_sound).pack(anchor='w')
    ttk.Checkbutton(misc_frame, text="Popup thông báo Thắng/Thua", variable=var_notify).pack(anchor='w')

    ttk.Label(misc_frame, text="Đường dẫn Tesseract:").pack(anchor='w', pady=(6,2))
    entry_tess = ttk.Entry(misc_frame, width=38)
    entry_tess.insert(0, pytesseract.pytesseract.tesseract_cmd)
    entry_tess.pack(anchor='w')
    def pick_tess():
        path = filedialog.askopenfilename(filetypes=[("Executable","*"), ("All files","*.*")])
        if path:
            entry_tess.delete(0, tk.END); entry_tess.insert(0, path)
    ttk.Button(misc_frame, text="🔍 Chọn Tesseract.exe...", command=pick_tess).pack(anchor='w', pady=(4,0))

    ttk.Label(misc_frame, text="Dãy hệ số (phân tách bằng dấu phẩy):").pack(anchor='w', pady=(6,2))
    entry_levels = ttk.Entry(misc_frame, width=38)
    entry_levels.insert(0, ",".join(map(str, BET_LEVELS)))
    entry_levels.pack(anchor='w')

    # Lưu references cho load_config/apply_updates
    widgets.update({
        "var_delay_bet": var_delay_bet,
        "var_delay_win": var_delay_win,
        "var_dry": var_dry,
        "var_sound": var_sound,
        "var_notify": var_notify,
        "entry_tess": entry_tess,
        "entry_levels": entry_levels,
    })

    # --- Nút Cập nhật & Lưu ---
    ttk.Button(right_frame, text="💾 Cập nhật & Lưu (config.json)",
               command=lambda: apply_updates(widgets, status_var, save=True, path=DEFAULT_CONFIG)).pack(pady=(4,6))

    # --- Điều khiển ---
    ttk.Label(right_frame, text="🎮 Điều khiển", font=('Arial', 10, 'bold')).pack(pady=(6, 4))
    ctrl = ttk.Frame(right_frame)
    ctrl.pack()
    ttk.Button(ctrl, text="▶ Start",
               command=lambda: start_bot(status_var, bet_var, log_box, spark_canvas)).grid(row=0, column=0, padx=3)
    ...
    # --- Nút mở Logs & Thoát ---
    foot = ttk.Frame(right_frame)
    foot.pack(pady=8)
    ttk.Button(foot, text="🗂 Mở thư mục Logs", command=lambda: subprocess.Popen(["explorer", os.path.abspath("logs")]) if os.name=="nt" else subprocess.Popen(["xdg-open", os.path.abspath("logs")])).grid(row=0, column=0, padx=4)
    ttk.Button(foot, text="🚪 Thoát", command=root.destroy).grid(row=0, column=1, padx=4)

    # --- Nạp config mặc định nếu có ---
...
def _parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Auto Bot Winluck v4_pro (Compact UI+)")
