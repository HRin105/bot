...
def main_gui():
    ...
    def m_save_config():
        path = filedialog.asksaveasfilename(initialdir="profiles", defaultextension=".json", filetypes=[("JSON","*.json")])
        if path:
            save_config_to(path, status_var)
    ...
    def m_load_config():
        ...
        if path:
            load_config_from(path, widgets, status_var)
    ...
    def m_browse_tess():
        ...
        if path:
            widgets["entry_tess"].delete(0, tk.END)
            widgets["entry_tess"].insert(0, path)
            status_var.set("🔍 Đã chọn đường dẫn Tesseract")

    filemenu.add_command(label="Save Config", command=m_save_config)
    filemenu.add_command(label="Load Config", command=m_load_config)
    filemenu.add_separator()
    filemenu.add_command(label="Chọn Tesseract.exe...", command=m_browse_tess)
    filemenu.add_separator()
    ...
    # ---------- LEFT SIDE ----------
    ttk.Label(left_frame, text="🤖 Trạng thái bot:", font=('Arial', 11, 'bold')).pack(pady=(0,4))
    ttk.Label(left_frame, textvariable=status_var, style="Status.TLabel").pack()
    ttk.Label(left_frame, textvariable=bet_var, font=('Arial', 10)).pack(pady=4)

    ttk.Label(left_frame, text="📊 Dashboard (60 gần nhất)", font=('Arial', 10, 'bold')).pack(pady=(10,4))
    spark_canvas = tk.Canvas(left_frame, width=400, height=80, bg="#1e1e1e",
                             highlightthickness=1, highlightbackground="#444")
    spark_canvas.pack(pady=(0,8))

    ttk.Label(left_frame, text="🧭 Nhật ký", font=('Arial', 10, 'bold')).pack(pady=(1,1))
    log_box = scrolledtext.ScrolledText(left_frame, width=54, height=20, font=('Consolas', 9))
    ...
    ttk.Label(right_frame, text="⚙️ Cập nhật tọa độ:", font=('Arial', 10, 'bold')).pack(pady=(0,4))
    ...
    ttk.Button(coord_frame, text="📍 Gán tọa độ (2s)",
               command=lambda: pick_position(focused_entry, widgets, status_var, root)).pack(pady=6)

    # --- Delay / Tùy chọn ---
    ttk.Label(right_frame, text="🕒 Cấu hình thời gian chờ:", font=('Arial', 10, 'bold')).pack(pady=(10,4))
    ...
    ttk.Label(delay_frame, text="⏱ Sau khi cược (s):").pack(anchor='w')
    ttk.Entry(delay_frame, textvariable=var_delay_bet, width=10).pack(anchor='w', pady=2)
    ttk.Label(delay_frame, text="📩 Sau khi thắng chờ (s):").pack(anchor='w')
    ttk.Entry(delay_frame, textvariable=var_delay_win, width=10).pack(anchor='w', pady=2)
    ...
    ttk.Checkbutton(misc_frame, text="Âm thanh thắng/thua (Windows)", variable=var_sound).pack(anchor='w')

    ttk.Label(misc_frame, text="Đường dẫn Tesseract:").pack(anchor='w', pady=(6,2))
    entry_tess = ttk.Entry(misc_frame, width=40)
    entry_tess.insert(0, pytesseract.pytesseract.tesseract_cmd)
    entry_tess.pack(anchor='w')

    ttk.Label(misc_frame, text="Dãy hệ số cược (phân cách phẩy):").pack(anchor='w', pady=(6,2))
    entry_levels = ttk.Entry(misc_frame, width=40)
    ...
    ctrl.pack(pady=8)
    ttk.Button(ctrl, text="▶ Start", width=8,
               command=lambda: start_bot(status_var, bet_var, log_box, spark_canvas)).pack(side='left', padx=4)
    ttk.Button(ctrl, text="⏸ Pause", width=8, command=pause_bot).pack(side='left', padx=4)
    ttk.Button(ctrl, text="⏵ Resume", width=8, command=resume_bot).pack(side='left', padx=4)
    ttk.Button(ctrl, text="⏹ Stop", width=8, command=lambda: stop_bot(status_var)).pack(side='left', padx=4)

    # --- Cập nhật ---
    btns_cfg = ttk.Frame(right_frame)
    btns_cfg.pack(pady=6)
    ttk.Button(btns_cfg, text="💾 Lưu config", width=15,
               command=lambda: apply_updates(widgets, status_var, save=True, path=DEFAULT_CONFIG)).pack(side='left', padx=4)
    ttk.Button(btns_cfg, text="🔄 Cập nhật tạm", width=15,
               command=lambda: apply_updates(widgets, status_var, save=False)).pack(side='left', padx=4)
